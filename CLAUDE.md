...
- プロジェクトの構造、各バージョンの詳細な機能および変更履歴については `/docs/versions/` 内の各バージョンのデータを参照すること。
- 最新バージョンを把握する際は、まず該当するバージョンの `structure.json` と `notes.md` を読み込むこと。

---

## バージョン管理ルール

バージョン履歴の詳細は `/docs/versions/` を参照すること。CLAUDE.md 本体にはバージョン履歴の詳細を記載しない。

### 新バージョン（vXXX）指定時の手順

新しいバージョンが指定された場合、以下を順番に実行する：

1. `/docs/versions/vXXX/` ディレクトリを作成する
2. `structure.json` を現在の実装状態から生成する
3. `notes.md` を日本語で作成する（変更点・決定事項・未実装を含む）

### structure.json の制約

- 実在するファイル・要素のみ記載する（推測で記述しない）
- 各項目に `status` フィールドを付与する
...
- 日本語で簡潔に記述する
- 技術的事実のみ記載する（推測・曖昧表現を使わない）
- 記載する項目: 変更点 / 決定事項 / 未実装

### 処理停止条件

不明点がある場合は処理を停止し、ユーザーに確認してから再開する。
